function [estimated3D] = superResolution2(param,HS_data,MS_data,T,patchsize)
...
%min_dim = min(M,N);
dim = floor([M,N]/patchsize)*patchsize;
%S = S(1:dim, 1:dim, :);
...
%--------------------------------------------------------------
%patchsize = 8;
total_patches = (dim(1)/patchsize)*(dim(2)/patchsize);
...
[ estimated3D ] = convert2Dto3Dpatchwise( S_bar_cap, patchsize , dim);
...
function [ mat3D ] = convert2Dto3Dpatchwise( im2D, n , dim)
[l N] = size(im2D);
x = dim(1);%sqrt(N);
y = dim(2);%sqrt(N);
indx = 1:n:x;
