% =========================================================================
% This code corresponds to the example with the Ivanpah Playa image 
% contained in the paper referenced below:
% 
% -------------------------------------------------------------------------
...
decimFactor = 4;
H_blur = conv2(ones(decimFactor)/(decimFactor^2), fspecial('gaussian',4,1)); 
...
% vbands = [100 80 50]; % RGB bands
% vbands = [90 30 10]; % RGB bands
vbands = [32 20 8]; % RGB bands --> visible spectra OK!
ddata = Zh_th;
...
% print(['examples/recIm_ex3b_Playa_vis'],'-dpdf')
...
vbands = [146 108 46]; % RGB bands --> IR OK!
ddata = Zh_th;
rgbim(:,:,1) = imadjust(rescale(ddata(:,:,vbands(1)),1));
rgbim(:,:,2) = imadjust(rescale(ddata(:,:,vbands(2)),1));
rgbim(:,:,3) = imadjust(rescale(ddata(:,:,vbands(3)),1));
...
% print(['examples/recIm_ex3b_Playa_IR'],'-dpdf')
...
figure;
[ha, pos] = tight_subplot(2, 5, [0.01 0.01], 0.35, 0.1);
...
% print(['examples/recIm_ex3b_Playa_vis_IR'],'-dpdf')
...
% repeat in a single figure
figure; clear rgbim
[ha, pos] = tight_subplot(2, 5, [0.01 0.01], 0.18, 0.1);

% vbands = [57,30,20]; % RGB bands
vbands = [32 20 8]; % RGB bands --> visible spectra OK!
ddata = Zh_th;
rgbim(:,:,1) = imadjust(rescale(fliplr(rot90(ddata(:,:,vbands(1)),-1)),1));
rgbim(:,:,2) = imadjust(rescale(fliplr(rot90(ddata(:,:,vbands(2)),-1)),1));
rgbim(:,:,3) = imadjust(rescale(fliplr(rot90(ddata(:,:,vbands(3)),-1)),1));
axes(ha(1)); imshow(rgbim); % display RGB image
set(gca,'xtick',[]), set(gca,'xticklabel',[]), set(gca,'ytick',[]), set(gca,'yticklabel',[])
title('Reference','interpreter','latex')
...
axes(ha(1)); ylabel('Visible','interpreter','latex')
axes(ha(6)); ylabel('Infrared','interpreter','latex')
% print(['examples/recIm_ex3b_Playa_vis_IR2'],'-dpdf')
...
ylabel('PSNR [dB]')
% print(['examples/PSNR_band_ex3b_Playa'],'-dpdf')
...
set(h,'fontsize',12);
% print(['examples/SAM_map_ex3b_Playa'],'-dpdf')
